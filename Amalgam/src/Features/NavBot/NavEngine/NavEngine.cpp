...
Vector CNavParser::handleDropdown(Vector current_pos, Vector next_pos)
{
	...
	if (height_diff < 0)
	{
		...
		if (drop_distance <= PLAYER_CROUCHED_JUMP_HEIGHT)
			return current_pos;
...
void CNavEngine::followCrumbs(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	if (G::Attacking != 1)
	{
		...
		default:
			F::BotUtils.LookAtPath(pCmd, Vec2{ crumbs[0].vec.x, crumbs[0].vec.y }, pLocal->GetEyePosition(), Vars::Misc::Movement::NavEngine::LookAtPath.Value == Vars::Misc::Movement::NavEngine::LookAtPathEnum::Silent);
			break;
		}
	}
